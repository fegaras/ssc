...
object Pretty extends RegexParsers {
  ...
  /** pretty-print trees */
  def pretty ( e: PPTree, position: Int, prefix: String ): String = {
    e match {
      case PPNode(f,l) if position+size(e) <= screen_size
        => l.map(pretty(_,position,prefix)).mkString(f+"(", ", ", ")")
      case PPNode(f,l)
        => l.map(pretty(_,position+f.length+1,prefix))
            .mkString(f+"(", ",\n"+prefix+" "*(position+f.length+1), ")")
      case TupleNode(l)
        => l.map(pretty(_,position,prefix)).mkString("( ", ", ", " )")
      case MapNode(l) if position+size(e) <= screen_size
        => l.map{ case (v,a) => pretty(v,position,prefix)+" = "+pretty(a,position,prefix) }.mkString("< ", ", ", " >")
      case MapNode(l)
        => l.map{ case (v,a) => pretty(v,position+2,prefix)+" = "+pretty(a,position+size(v)+3,prefix) }
            .mkString("< ", ",\n"+prefix+" "*(position+2), " >")
      case LeafS(v)
        => v.toString
      case LeafD(d)
 ...
 /** pretty-print the printout of a case class object */
  def print ( x: Any, prefix: String = "" ): String = {
    parseAll(tree,x.toString) match {
      case Success(t,_) => prefix+pretty(t,0,prefix)
